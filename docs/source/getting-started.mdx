...
We'll also install the [`graphql-tag`](https://www.npmjs.com/package/graphql-tag) package, which includes a template literal tag that parses GraphQL query strings into the standard GraphQL AST.
...
If you are using JavaScript, create a `src/index.js` file. If you plan to use TypeScript, create a `src/index.ts` file:
...
<ExpansionPanel title="Learn how to set up TypeScript (Optional)">
...
3. You can now run the `tsc --init` command, which creates a `tsconfig.json` file in your project:
...
If you are using JavaScript (i.e., not TypeScript), add the following script to your `package.json`:
...
// function enables you to start listening to your server
