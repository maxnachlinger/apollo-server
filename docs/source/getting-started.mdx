...
JavaScript, and that you have a recent version of Node.js (14+) installed. Note, this tutorial walks through how to set up Apollo Server using either JavaScript or TypeScript.
...
We'll also install the [`graphql-tag`](https://www.npmjs.com/package/graphql-tag) package, which includes a template literal tag that parses GraphQL query strings.
...
<ExpansionPanel title="Expand to learn how to set up TypeScript (Optional)">

#### Set up TypeScript

1. If you do not already have TypeScript installed on your computer, install it globally now:
```bash
npm install -g typescript
```

2. Run the following command to install TypeScript into your project's dependencies:

```bash
npm install typescript
```

3. You can now run the `tsc --init` command, which creates a `tsconfig.json` file:

```bash
tsc --init
```

4. The `tsconfig.json` file enables us to configure how TypeScript will compile our code. Add the following configuration to your `tsconfig.json`:
...
5. Finally, add a `type` and `scripts` to the `package.json` file you previously created:
...
The above `start` script will compile your code into TypeScript before running it with `node`. Setting your project's [`type` to `module`](https://nodejs.org/api/packages.html#approach-1-use-an-es-module-wrapper) will transpile ES module sources into `CommonJS`, enabling you to use top-level [`await`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) calls.

</ExpansionPanel>

If you are using JavaScript, add the below following script to your `package.json`:
