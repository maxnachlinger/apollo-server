...
Apollo Server 4's new [`startStandaloneServer`](#startstandaloneserver) function (e.g.,the replacement for the batteries-included `apollo-server` package) includes both the `body-parser` and `cors` packages under the hood, but neither package is configurable.
...
The `ExecutionResult` type from `graphql-js` has replaced the similar `GraphQLExecutionResult` type (previously returned by `executor`). The types are essentially the same, except `data` and `extensions` are now `Record<string, unknown>`, rather than `Record<string, any>`.

## Changes to framework integrations

Apollo Server 4 [removes built-in framework integrations](#removed-integrations) and changes the Apollo Server API by no longer subclassing `ApolloServer`.
...
Apollo Server 4 introduces the [`expressMiddleware`](https://github.com/apollographql/apollo-server/blob/main/packages/server/src/express/index.ts) function, enabling you to attach an `ApolloServer` to an [Express](https://expressjs.com/) server from the start using the `@apollo/server` package.
...
In Apollo Server 4, we introduce the `startStandaloneServer` function in the main `@apollo/server` package. The `startStandaloneServer` function has a similar concept to the "batteries-included" `ApolloServer`,
...
The `startStandaloneServer` function accepts three arguments:
  1. An instance of `ApolloServer` to start and listen to.
  2. `listenOpts` accepts the same arguments as [http.Server's listen() method](https://nodejs.org/dist/latest-v12.x/docs/api/http.html#http_server_listen) method.
  3. `serverOpts`
...
In Apollo Server 4, we refactored  the `GraphQLResponse` object (e.g., `requestContext.response`). The `data`, `errors`, and `extensions` fields are now on a `result` sub-object. The `data` and `extensions` fields are now `Record<string, unknown>`, rather than `Record<string, any>`, though more casts may be needed. `GraphQLRequestContext.response` and `GraphQLResponse.http` are now always set.
...
- double check removed all `ApolloServerStandAlone `mentions with `startStandaloneServer` function
- double check all imports
- Ensure covered everything
- identify what to ask glasser/trevor about


Questions for Trevor/Glasser:
  - New HTTP request execution API
  - Can I get a list of the full
  - In general we just need to go over the full exported API and list what’s gone and where it’s gone. (This includes the re-export of `gql` from `graphql-tag`)
    - Should this be done for the alpha?
>>>>>>> eca61a86d (Continue documenting startStandaloneServer)
