...
// interface for how the context should be typed
const server = new ApolloServer<MyContext>({
  typeDefs,
  resolvers,
});
...
const { url } = await startStandaloneServer(server, serverOpts, listenOpts)
```
...
The `startStandaloneServer` function accepts two arguments:

| Argument  | Description  |
|---|---|
| `server` | <p>The instance of `ApolloServer` that should begin listening for incoming requests.</p>  |
| `options` | <p>The options for your server. This is commonly where you will define a `context` _function_ which returns an object to use as your [GraphQL `context` object](/apollo-server/data/resolvers/#the-context-argument).</p> |

Within the options object that `startStandaloneServer` function accepts:

| Argument  | Description  |
|---|---|
| `serverOpts` | <p>The options for your server. This is commonly where you will define a `context` _function_ which returns an object to use as your [GraphQL `context` object](/apollo-server/data/resolvers/#the-context-argument).</p> |
| `listenOpts` |  <p>The options passed to the `listen` method of the under-the-hood `http.Server`. <p></p>Supported options are listed in the documentation for [net.Server.listen](https://nodejs.org/api/net.html#net_server_listen_options_callback).</p> |

Below is an example of the high level set up for using the `startStandaloneServer` function:
...
const { url } = await startStandaloneServer(apolloServerInstance, {
  // The context function is run once for every incoming GraphQL request
  context: async ({req, _res}) => ({
    token: await getTokenForRequest(req),
  }),
  listen: { port: 4000 }
...
To provide an initial [`context`](apollo-server/data/resolvers/#the-context-argument) for your server (and underlying resolvers) with the `startStandaloneServer` function, pass in a second argument

to your resolvers, add a context initialization function to the ApolloServer constructor. This function is called with every request, so you can customize the context based on each request's details (such as HTTP headers).

The second argument passed to `startStandaloneServer` enables you to add a `context` function that will run once
