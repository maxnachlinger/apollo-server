...
Currently the [`expressMiddleware`](https://github.com/apollographql/apollo-server/blob/main/packages/server/src/express/index.ts) function has no runtime dependency on `express` (or any other package) though `standaloneServer` does.
