...
Apollo Server 3 returned certain errors relating to GraphQL operations over `HTTP`/`JSON` as `text/plain` error messages.
...
Apollo Server v3.2.0 added a "draining" phase to server shutdown, which enables a server to complete in-progress operations before continuing to shut down. Without this draining phase, operations can reach your server even after it has stopped causing those operations to fail.

In Apollo Server 4, if a server receives an operation after the "draining" phase, it will log a warning encouraging you to set up draining before failing the operation.

Note, those using the `startStandaloneServer` function have their server drained automatically. If you are using `expressMiddleware` or another `http.Server`-based web servers, you can add "draining" using the  [`ApolloServerPluginDrainHttpServer` plugin](/apollo-server/api/plugin/drain-http-server/#using-the-plugin).

### `GraphQLResponse`

In Apollo Server 4, we refactored the [`GraphQLResponse` object](https://github.com/apollographql/apollo-server/blob/version-4/packages/server/src/externalTypes/graphql.ts#L25) (e,g., `requestContext.response`). The `data`, `errors`, and `extensions` fields are now within an object from the `result` field:
...
The `data` and `extensions` fields are both now opf type `Record<string, unknown>`, rather than `Record<string, any>`. Additionally, `GraphQLRequestContext.response` and `GraphQLResponse.http` are always set.
