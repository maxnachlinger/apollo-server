...
To provide an initial [`context`](apollo-server/data/resolvers/#the-context-argument) to your resolvers using `startStandaloneServer`, add a `context` initialization function to your server's options. This function is called with every request, so you can customize the context based on each request's details (such as HTTP headers).
...
Apollo Server 4 clear this up by requiring that within a `POST` request's JSON-encoded body, the `variables` and `extensions` fields must be objects. Additionally, neither the `variables` nor `extensions` fields should be doubly-encoded.

For example, below is a valid query in Apollo Server 4:
```
{“query”: “{ __typename }”, extensions: {“foo”: 1}}
```

While this query would be invalid:
```
{“query”: “{ __typename }”, extensions: “{\“foo\”: 1}”}
```
...
### Improved TypeScript typing for context creation
