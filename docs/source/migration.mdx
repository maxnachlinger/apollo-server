...
The `Cache` and `keyv` binding can now be found in the utils monorepo, published under the packages:
...
Existing implementations of `KeyValueCache` should continue to work. Additionally, the default cache is now bounded, any previous mentions of `cache: 'bounded'` can now be deleted.
