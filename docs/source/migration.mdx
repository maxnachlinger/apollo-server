...
In Apollo Server 4,  serverless integrations differentiate themselves from other integrations by calling the `startInBackgroundHandlingStartupErrorsByLoggingAndFailingAllRequests()` method. The length of this function's name discourages its use when building non-serverless apps (because the shorter `await server.start()` is available).

This means those using serverless middleware shouldn't have to call any start-related functions, like so:

```ts
exports.handler = lambdaHandler(server);
```
In the above example, the `lambdaHandler` serverless middleware function should call
